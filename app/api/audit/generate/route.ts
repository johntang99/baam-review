...
interface GenerateRequest {
  ...
  /** Server-side snapshot id generated by /api/audit/competitors/preview. */
...
export async function POST(request: Request) {
  ...
  const competitorScenarioId = parseCompetitorScenarioId(
    body.competitor_scenario_id,
  );
  ...
  if (competitorScenarioId) {
    const scenario = await readCompetitorScenario({
      scenario_id: competitorScenarioId,
      user_id: auth.user.id,
    }).catch((err) => {
      console.error("[audit/generate] read scenario failed:", err);
      return null;
    });
    if (!scenario) {
      return NextResponse.json(
        { error: "competitor_scenario_missing" },
        { status: 400 },
      );
    }
    const canonicalScenarioService = canonicalizeService(
      scenario.service_override_canonical || scenario.service_override,
    );
    if (
      canonicalScenarioService &&
      canonicalScenarioService !== canonicalServiceOverride
    ) {
      return NextResponse.json(
        { error: "competitor_selection_stale" },
        { status: 400 },
    ...
    if (scenario.status === "hydrating") {
      return NextResponse.json(
        { error: "competitor_preview_in_progress" },
        { status: 400 },
      );
    }
    if (scenario.status === "failed") {
      return NextResponse.json(
        { error: "competitor_preview_failed" },
        { status: 400 },
      );
    }
    const hydratedSet = new Set(scenario.hydrated_place_ids);
    const missingHydrated = selectedCompetitorPlaceIds.some(
  ...
  const pipelineInput = {
    ...
    competitor_scenario_id: competitorScenarioId,
...
function parseCompetitorScenarioId(input: string | undefined) {
