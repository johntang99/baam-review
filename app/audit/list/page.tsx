...
export default async function AuditListPage(props: {
  ...
  // Resolve who generated each audit. The `users` table RLS limits reads to the
  // viewer's own account, so an admin viewing every audit can't join through it.
  // The audit rows above are already RLS-authorized, so we look up just the
  // generators' display names with the service-role client (display only).
  ...
  if (generatorIds.length > 0) {
    ...
    const { data: gens } = await admin
      ...
      .in("id", generatorIds);
    for (const g of gens ?? []) {
      generatorMap.set(g.id, g.full_name?.trim() || "—");
    }
  }
              ...
              {sortedRows.map(({ row, isReAudit, trendDelta, isBilingual }) =>
                row.status === "generating" ? (
                  ...
                  <CompletedRow
                    ...
                    generatorName={
                      row.user_id ? generatorMap.get(row.user_id) ?? "—" : "—"
                    }
                  />
                ),
              )}
...
function CompletedRow({
  ...
  const generatorInitial = generatorName.trim().charAt(0).toUpperCase() || "?";
  ...
  return (
    <div className="audit-row">
      ...
      <div className="audit-row-user-block" title={`Generated by ${generatorName}`}>
        <span className="audit-row-user-avatar">{generatorInitial}</span>
...
function GeneratingRow({ row }: { row: AuditRow }) {
  ...
  return (
    <div className="audit-row simple" style={{ opacity: 0.85 }}>
      <div className="audit-row-info">
        <div className="audit-row-name-row">
...
function FailedRow({ row }: { row: AuditRow }) {
  return (
    <div className="audit-row simple" style={{ opacity: 0.7 }}>
      <div className="audit-row-info">
        <div className="audit-row-name-row">
